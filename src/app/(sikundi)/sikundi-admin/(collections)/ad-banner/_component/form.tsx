    ...
    const { isLoading, execute } = useAction(type === "create" ? AdCreateAction : AdUpdateAction, {
        onSuccess: ({ data }) => {
            if (data.action === "pending") {
                router.push('/sikundi-admin/ad-banner/copydesk')
            } else if (data.action === "soft_delete") {
                router.push('/sikundi-admin/ad-banner/trash')
            } else {
                    ...
                    <CardContent className="grid gap-4 aspect-video relative">
                        {
                            ...
                            : <Fragment>
                                <Image fill sizes="75vw" src={image} alt="feature image" className="w-full h-full object-contain" />
